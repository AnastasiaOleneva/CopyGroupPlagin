    ...
    public class CopyGroup : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                Reference reference = uiDoc.Selection.PickObject(ObjectType.Element, groupFilter ,"Выберите группу объектов");
                ...
                XYZ point2 = selectRoomCenter+offset;
                Transaction transaction = new Transaction(doc);
                using (Transaction ts = new Transaction(doc))
                {
                    transaction.Start("Копирование объектов");

                    Group groopCopy = doc.Create.PlaceGroup(point2, group.GroupType);
                    groopCopy.get_Parameter(BuiltInParameter.GROUP_OFFSET_FROM_LEVEL).Set(0);
                    transaction.Commit();
